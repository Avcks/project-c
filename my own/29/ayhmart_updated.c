...
/* ── Category manipulation forward declarations ── */
void categoryManage();
void addCategory();
void deleteCategory();
void viewCategories();
...
void mainManu() {
    ...
    while (1) {
        ...
        printDash();
        printf(C_YEL "  Enter your choice: " C_RST);
        scanf("%d", &choice);
        ...
        switch (choice) {
            ...
            case 0:
                ...
                exit(0);
            default:
                printf("  " C_YEL "[!]" C_RST " Invalid option. Try again.\n");
                system("pause");
        }
    }
}
...
void login() {
    ...
    if (strcmp(username, ad_user) == 0 && strcmp(password, ad_pass) == 0) {
        printf("\n  " C_GRN "[✓]" C_RST " Logged in as ADMIN\n");
        system("pause");
        adminDashboard();
    } else if (strcmp(username, sf_user) == 0 && strcmp(password, sf_pass) == 0) {
        printf("\n  " C_GRN "[✓]" C_RST " Logged in as STAFF\n");
        system("pause");
        staff();
    } else {
        printf("\n  " C_RED "[✗]" C_RST " Incorrect username or password.\n");
        system("pause");
    }
}
...
void adminDashboard() {
    ...
    switch (choice) {
        ...
        case 5: 
			...
			break;
        case 0:
            printf("\n  " C_GRN "[✓]" C_RST " Logged out successfully.\n\n");
            system("pause");
            mainManu();
            break;
        default:
            printf("  " C_YEL "[!]" C_RST " Invalid option.\n");
            system("pause");
            adminDashboard();
    }
}
...
void add() {
    ...
    for (i = 0; i < count; i++) {
        if (nProducts >= MAX_ITEMS) {
            printf("  " C_YEL "[!]" C_RST " Product limit reached (%d max).\n", MAX_ITEMS);
            system("pause");
            inventory();
            return;
        }
		...
		while (1) {
 		  	...
 		  	scanf("%d", &code);
    	    	...
    	    	printf("  " C_YEL "[!]" C_RST " Product code already exists. Enter different code.\n");
    		} else {
		...
		while (1) {
			...
			if (isProductNameExists(tempName, -1)) {
				printf("  " C_YEL "[!]" C_RST " Product with this name already exists. Enter different name.\n");
			} else {
		...
		if (nCategories == 0) {
            printf("  " C_YEL "[!]" C_RST " No categories available. Please add a category from Category Management first.\n");
            system("pause");
            inventory();
            return;
        } else {
            ...
            while (1) {
                printf("  Category: "); 
                scanf(" %29s", tempCategory);
                
                int valid = 0;
                for (int j = 0; j < nCategories; j++) {
                    if (strcasecmp(categories[j], tempCategory) == 0) {
                        strcpy(shop[nProducts].pCategory, categories[j]);
                        valid = 1;
                        break;
                    }
                }
                if (valid) {
                    ...
                    printf("  " C_YEL "[!]" C_RST " Invalid category. Please choose from the available categories.\n");
                }
            }
        }
        nProducts++;
    }
    printf("\n  " C_GRN "[✓]" C_RST " %d product(s) added successfully.\n", count);
    saveProducts();
    system("pause");
    inventory();
}
        ...
        if (shop[i].pCode == code) {
            ...
            while (1) {
                ...
                if (isProductNameExists(buf, i)) {
                    printf("  " C_YEL "[!]" C_RST " Product with this name already exists. Try different name.\n");
                } else {
            ...
            while (1) {
                ...
                int valid = 0;
                for (int j = 0; j < nCategories; j++) {
                    if (strcasecmp(categories[j], buf) == 0) {
                        strcpy(shop[i].pCategory, categories[j]);
                        valid = 1;
                        break;
                    }
                }
                if (valid) {
                    ...
                    printf("  " C_YEL "[!]" C_RST " Invalid category. Please choose from available categories.\n");
                    for (int j = 0; j < nCategories; j++) {
                        printf("      - %s\n", categories[j]);
                    }
                }
            ...
            printf("\n  " C_GRN "[✓]" C_RST " Product updated successfully.\n");
            saveProducts();
            break;
        }
    }
    if (!found){
	printf("  " C_RED "[✗]" C_RST " Product code not found.\n");
	} 
    system("pause");
    inventory();
}
...
void del() {
    ...
    for (i = 0; i < nProducts; i++) {
        if (shop[i].pCode == code) {
            ...
            nProducts--;
            found = 1;
            printf("  " C_GRN "[✓]" C_RST " Product deleted successfully.\n");
            saveProducts();
            break;
        }
    }
...
int customerLogin() {
    ...
    printf("\n  " C_RED "[✗]" C_RST " Invalid Customer ID or Password.\n");
    system("pause");
    return -1;
}
...
void search() {
    ...
    if (!found){
		printf("  " C_RED "[✗]" C_RST " Product not found.\n");
	} 
    printf("\n");
    system("pause");
    cust();
...
void addCart() {
    ...
    if (cartCount >= MAX_CART) {
        printf("  " C_YEL "[!]" C_RST " Cart is full!\n");
        system("pause");
        cust();
        return;
    }
    ...
    for (i = 0; i < nProducts; i++) {
        if (shop[i].pCode == code) {
            if (shop[i].pStock < qty) {
                printf("  " C_YEL "[!]" C_RST " Insufficient stock (available: %d).\n", shop[i].pStock);
                found = 1;
                break;
            }
            ...
            if (strcmp(confirm, "yes") == 0 || strcmp(confirm, "y") == 0) {
                ...
                saveProducts();
                cartCount++;
                printf("  " C_GRN "[✓]" C_RST " Item added to basket.\n");
            } else {
    ...
    if (!found){
		printf("  " C_RED "[✗]" C_RST " Product unavailable.\n");
	}
    printf("\n");
    system("pause");
    cust();
...
void checkout() {
    ...
    printHeader("AYH MART  |  Surat, India");
    if (cartCount == 0) {
        printf("  " C_YEL "[!]" C_RST " Basket is empty.\n");
        system("pause");
        cust();
        return;
    }
    ...
    printf(C_YEL "  Enter choice: " C_RST);
    scanf("%d", &payChoice);
...
void billing(float finalAmount, int isQR) {
    ...
    if (payAmount >= finalAmount) {
        ...
        printf("\n");
        printLine();
        printf("  " C_GRN "[✓]" C_RST " Payment received! Change: %.2f\n", change);
        printf("  Thank you for shopping at AYH Mart!\n");
        printLine();
        system("pause");
        mainManu();
    } else {
        printf("  " C_YEL "[!]" C_RST " Insufficient. Need %.2f more.\n", finalAmount - payAmount);
        billing(finalAmount, 0);
    }
}
...
void addMembers() {
    ...
    for (i = 0; i < count; i++) {
        if (nMembers >= MAX_MEMBERS) {
            printf("  " C_YEL "[!]" C_RST " Member limit reached.\n");
            break;
        }
        ...
        if (custIndex == -1) {
            printf("  " C_RED "[✗]" C_RST " Customer not found.\n");
            i--;
            continue;
        }else{
        ...
        if (customers[custIndex].cMemberId != 0) {
            printf("  " C_YEL "[!]" C_RST " This customer already has a membership.\n");
            i--;
            continue;
        }
        ...
        while (1) {
            ...
            if (c == 's') {
                ...
                printf("  " C_YEL "[!]" C_RST " Invalid tier. Try again.\n");
            }
        }
    ...
    printf("\n  " C_GRN "[✓]" C_RST " Member(s) added and linked successfully.\n");
    saveMembers();
    saveCustomers();
    system("pause");
    members();
...
void deleteMembers() {
    ...
    for (i = 0; i < nMembers; i++) {
        if (Members[i].mId == id) {
            ...
            nMembers--;
            found = 1;
            printf("  " C_GRN "[✓]" C_RST " Member deleted.\n");
            saveMembers();
            break;
        }
    }
    if (!found){
		printf("  " C_RED "[✗]" C_RST " Member ID not found.\n");
	}
    system("pause");
    members();
}
...
void addCustomer() {
    ...
    for (i = 0; i < count; i++) {
        if (nCustomers >= MAX_CUSTOMERS) {
            printf("  " C_YEL "[!]" C_RST " Customer limit reached (%d max).\n", MAX_CUSTOMERS);
            break;
        }
        printf("\n  -- Customer Account %d --\n", i + 1);
		int id;
		while (1) {
		    ...
		    if (isCustomerIdExists(id)) {
		        printf("  " C_YEL "[!]" C_RST " Customer ID already exists. Try again.\n");
		    } else {
		...
		printf("  Phone Number : ");
		scanf("%s", customers[nCustomers].cPhone);
		
		/* NEW: Email address input */
		printf("  Email Address: ");
		scanf("%s", customers[nCustomers].cEmail);
        ...
        while (1) {
            ...
            if (strcmp(customers[nCustomers].cPass, passConfirm) == 0) {
                ...
                printf("  " C_YEL "[!]" C_RST " Passwords do not match. Try again.\n");
            }
        }
		printf("\n");
    ...
    printf("\n  " C_GRN "[✓]" C_RST " Customer account(s) created successfully.\n");
    saveCustomers();
    system("pause");
    custManage();
}
...
void deleteCustomer() {
    ...
    for (i = 0; i < nCustomers; i++) {
        if (customers[i].cId == id) {
            ...
            nCustomers--;
            found = 1;
            printf("  " C_GRN "[✓]" C_RST " Customer account deleted.\n");
            saveCustomers();
            break;
        }
    }
    if (!found) {
		printf("  " C_RED "[✗]" C_RST " Customer ID not found.\n");
	}
    system("pause");
    custManage();
}
...
void viewMyCard() {
    ...
    if (linkedId == 0) {
        printf("  " C_YEL "[!]" C_RST " No membership card linked to your account.\n");
        ...
        if (!found) {
            printf("  " C_YEL "[!]" C_RST " Linked Member ID (%d) not found in records.\n", linkedId);
    ...
    for (i = 0; i < nMembers; i++) {
        if (Members[i].mId == memberId) {
            if (Members[i].mUsed == 1) {
                printf("\n  " C_YEL "[!]" C_RST " Membership card already used. No discount applied.\n");
                return 0;
            }
            ...
            Members[i].mUsed   = 1;
            printf("  " C_GRN "[✓]" C_RST " Membership discount applied (%.0f%%). Card is now marked as used.\n",
                   Members[i].mDisc);
            return discount;
        }
    }

    printf("  " C_YEL "[!]" C_RST " Member ID not found. No discount applied.\n");
    return 0;
}
...
void saveProducts() {
    ...
    if (!fp) {
        printf("  " C_RED "[✗]" C_RST " Error: Could not open products file for writing.\n");
        return;
    }
...
void saveMembers() {
    ...
    if (!fp) {
        printf("  " C_RED "[✗]" C_RST " Error: Could not open members file for writing.\n");
        return;
    }
...
void saveCustomers() {
    ...
    if (!fp) {
        printf("  " C_RED "[✗]" C_RST " Error: Could not open customers file for writing.\n");
        return;
    }
...
void saveSales() {
    ...
    if (!fp) {
        printf("  " C_RED "[✗]" C_RST " Error: Could not open sales file for writing.\n");
        return;
    }
...
void backupRestore() {
    ...
    switch (choice) {
        case 1:
            saveAllData();
            printf("\n  " C_GRN "[✓]" C_RST " All data saved successfully.\n");
            printf("      Products  -> %s\n", FILE_PRODUCTS);
            printf("      Members   -> %s\n", FILE_MEMBERS);
            printf("      Customers -> %s\n", FILE_CUSTOMERS);
            printf("      Sales     -> %s\n", FILE_SALES);
        ...
        case 2: {
            char confirm[5];
            printf("\n  " C_YEL "[!]" C_RST " This will overwrite current in-memory data.\n");
            printf("  Are you sure? (yes/no): ");
            scanf("%s", confirm);
            if (strcmp(confirm, "yes") == 0 || strcmp(confirm, "y") == 0) {
                loadAllData();
                printf("\n  " C_GRN "[✓]" C_RST " Data restored from files.\n");
                printf("      Products  : %d loaded\n",  nProducts);
                printf("      Members   : %d loaded\n",  nMembers);
                printf("      Customers : %d loaded\n",  nCustomers);
                printf("      Sales     : %d loaded\n",  nSales);
                printf("      Categories: %d loaded\n",  nCategories);
            } else {
...
/* NEW: Check if product code exists */
int isProductCodeExists(int code) {
...
/* NEW: Check if product name exists */
int isProductNameExists(char name[], int excludeIndex) {
    for (int i = 0; i < nProducts; i++) {
        if (i == excludeIndex) continue;
        if (strcasecmp(shop[i].pName, name) == 0) {
            return 1;
        }
    }
    return 0;
...
/* NEW: Check if customer ID exists */
int isCustomerIdExists(int id) {
...
void addCategory() {
    ...
    if (nCategories >= MAX_CATEGORIES) {
        printf("  " C_YEL "[!]" C_RST " Category limit reached (%d max).\n", MAX_CATEGORIES);
        system("pause");
        categoryManage();
        return;
    }
    ...
    for (int i = 0; i < nCategories; i++) {
        if (strcasecmp(categories[i], newCat) == 0) {
            printf("  " C_YEL "[!]" C_RST " Category already exists.\n");
            system("pause");
            categoryManage();
            return;
        }
    ...
    strcpy(categories[nCategories], newCat);
    nCategories++;
    
    printf("\n  " C_GRN "[✓]" C_RST " Category added successfully.\n");
    saveCategories();
    system("pause");
    categoryManage();
}
...
void deleteCategory() {
    ...
    for (int i = 0; i < nCategories; i++) {
        if (strcasecmp(categories[i], delCat) == 0) {
            /* Check if any product is using this category */
            int inUse = 0;
            for (int p = 0; p < nProducts; p++) {
                if (strcasecmp(shop[p].pCategory, delCat) == 0) {
                    inUse = 1;
                    break;
                }
            }
            ...
            if (inUse) {
                printf("  " C_YEL "[!]" C_RST " Cannot delete category. It is assigned to one or more products.\n");
            } else {
                for (int j = i; j < nCategories - 1; j++) {
                    strcpy(categories[j], categories[j + 1]);
                }
                nCategories--;
                printf("  " C_GRN "[✓]" C_RST " Category deleted successfully.\n");
                saveCategories();
            }
            found = 1;
            break;
    ...
    if (!found) {
        printf("  " C_RED "[✗]" C_RST " Category not found.\n");
    }
...
void saveCategories() {
    ...
    if (!fp) {
        printf("  " C_RED "[✗]" C_RST " Error: Could not open categories file for writing.\n");
        return;
    }
    fwrite(&nCategories, sizeof(int), 1, fp);
    fwrite(categories, sizeof(char) * 30, nCategories, fp);
    fclose(fp);
}
...
void loadCategories() {
    ...
    fread(categories, sizeof(char) * 30, nCategories, fp);
    fclose(fp);
}
