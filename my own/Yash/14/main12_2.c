...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• INPUT SAFEGUARDS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
/* â”€â”€ Customer account forward declarations â”€â”€ */
void custManage();
void addCustomer();
void deleteCustomer();
void viewCustomers();
int  customerLogin();
void viewMyCard();
/* â”€â”€ File handling forward declarations â”€â”€ */
void saveProducts();
void loadProducts();
void saveMembers();
void loadMembers();
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• STRUCTURES â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
struct Product {
    int   pCode;
    char  pName[50];
    float pPrice;
...
int totalBills = 0;
/* â”€â”€ Customer account structure â”€â”€ */
struct Customer {
    int  cId;
    char cName[50];
    char cPass[20];
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• STYLING HELPERS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void printLine() {
    printf("============================================================\n");
}
void printDash() {
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• ADMIN DASHBOARD â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void adminDashboard() {
    int choice;
    system("cls");
    printHeader("AYH MART  |  Surat, India");
...
void add() {
    ...
    for (i = 0; i < count; i++) {
        ...
        printf("  Price   : "); 
		SAFE_INPUT_FLOAT(shop[nProducts].pPrice);
        printf("  Stock   : "); 
		SAFE_INPUT_INT(shop[nProducts].pStock);
        printf("  Category: "); 
		scanf("%s",   shop[nProducts].pCategory);
        nProducts++;
    }
    printf("\n  [âœ“] %d product(s) added successfully.\n", count);
    saveProducts();
    system("pause");
    inventory();
}
...
void editProduct() {
    ...
    if (!found) printf("  [âœ—] Product code not found.\n");
    system("pause");
    inventory();
}
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• STORE INFO â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void info() {
    system("cls");
    printHeader("AYH MART  |  Surat, India");
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• CUSTOMER LOGIN â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
int customerLogin() {
    int  id, i;
    char pass[20];
    system("cls");
    ...
    printf("\n  [âœ—] Invalid Customer ID or Password.\n");
    system("pause");
    return -1;
}

/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• CUSTOMER MENU â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void cust() {
    int choice, i;
...
void search() {
    ...
    if (!found) printf("  [âœ—] Product not found.\n");
    printf("\n");
    system("pause");
    cust();
}
...
void addCart() {
    ...
    for (i = 0; i < nProducts; i++) {
        if (shop[i].pCode == code) {
            ...
            if (strcmp(confirm, "yes") == 0 || strcmp(confirm, "y") == 0) {
                ...
                shop[i].pStock       -= qty;
                saveProducts();   /* persist stock deduction immediately */
                cartCount++;
                printf("  [âœ“] Item added to basket.\n");
            } else {
    ...
    if (!found) printf("  [âœ—] Product unavailable.\n");
    printf("\n");
    system("pause");
    cust();
}
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• PRINT QR CODE â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
/*
...
void checkout() {
    ...
    printf("  Enter choice: ");
    SAFE_INPUT_INT(payChoice);

    for (i = 0; i < cartCount; i++) {
        recordSale(cart[i].code, cart[i].name, cart[i].qty, cart[i].price, customers[currentCustIdx].cId);
    }

    /* â”€â”€ Increment bill counter and persist BEFORE billing() navigates away â”€â”€ */
    totalBills++;
    cartCount      = 0;
    custLoggedIn   = 0;
    currentCustIdx = -1;
    ...
    if (payChoice == 2) {
        /* â•â• UPDATED QR CODE â€” matches Dhameliya Yash Mukeshbhai's actual UPI QR â•â• */
        printf("\n  [ SCAN QR CODE TO PAY â€” Dhameliya Yash Mukeshbhai ]\n\n");
        printQRCode();
        printf("\n  UPI ID : 6353999107@ptyes\n");
        printf("  Amount : Rs. %.2f\n\n", finalAmount);
        totalQRPayments++;
...
void billing(float finalAmount, int isQR) {
    ...
    SAFE_INPUT_FLOAT(payAmount);

    if (payAmount >= finalAmount) {
        float change = payAmount - finalAmount;
        printf("\n");
        printLine();
        printf("  [âœ“] Payment received! Change: %.2f\n", change);
        printf("  Thank you for shopping at AYH Mart!\n");
        printLine();
        system("pause");
        mainManu();
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• MEMBERS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void members() {
    int choice;
    system("cls");
    printHeader("AYH MART  |  Surat, India");
...
void addMembers() {
    ...
    for (i = 0; i < count; i++) {
        ...
        SAFE_INPUT_INT(custId);

        /* â”€â”€ Find Customer â”€â”€ */
        ...
        if (custIndex == -1) {
            printf("  [âœ—] Customer not found.\n");
            i--;
            continue;
        }else{
        	/* â”€â”€ Show Purchase History â”€â”€ */
        ...
        /* â”€â”€ Prevent Duplicate Membership â”€â”€ */
        ...
        /* â”€â”€ Auto Assign Member Details â”€â”€ */
        ...
        printf("  Amount Spent: ");
        SAFE_INPUT_FLOAT(Members[nMembers].mSpent);
        ...
        /* â”€â”€ Link Membership to Customer â”€â”€ */
        customers[custIndex].cMemberId = Members[nMembers].mId;

        nMembers++;
    }

    printf("\n  [âœ“] Member(s) added and linked successfully.\n");
    saveMembers();
    saveCustomers();
    system("pause");
    members();
...
void deleteMembers() {
    ...
    for (i = 0; i < nMembers; i++) {
        if (Members[i].mId == id) {
            ...
            nMembers--;
            found = 1;
            printf("  [âœ“] Member deleted.\n");
            saveMembers();
            break;
        }
    }
    if (!found) printf("  [âœ—] Member ID not found.\n");
    system("pause");
    members();
}
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• CUSTOMER ACCOUNT MANAGEMENT â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
void addCustomer() {
    ...
    for (i = 0; i < count; i++) {
        ...
        SAFE_INPUT_INT(customers[nCustomers].cMemberId);

        nCustomers++;
    }
    printf("\n  [âœ“] Customer account(s) created successfully.\n");
    saveCustomers();
    system("pause");
    custManage();
}

/* â”€â”€ Delete customer account â”€â”€ */
void deleteCustomer() {
    int id, i, j, found = 0;
    system("cls");
    printHeader("AYH MART  |  Surat, India");
    ...
    for (i = 0; i < nCustomers; i++) {
        if (customers[i].cId == id) {
            ...
            nCustomers--;
            found = 1;
            printf("  [âœ“] Customer account deleted.\n");
            saveCustomers();
            break;
        }
    }
    if (!found) printf("  [âœ—] Customer ID not found.\n");
    system("pause");
    custManage();
}

/* â”€â”€ View all customer accounts â”€â”€ */
void viewCustomers() {
    int i;
    system("cls");
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• VIEW MY MEMBERSHIP CARD â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void viewMyCard() {
    int i, found = 0;
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• SALES â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void recordSale(int code, char name[], int qty, float price, int custId) {
    ...
    totalRevenue += sales[nSales].total;  /* keep global counter in sync */

    nSales++;
}

/* â”€â”€ applyDiscount: one-time card restriction â”€â”€ */
float applyDiscount(int memberId, float billAmount) {
    ...
    for (i = 0; i < nMembers; i++) {
        if (Members[i].mId == memberId) {
            ...
            Members[i].mUsed   = 1;
            printf("  [âœ“] Membership discount applied (%.0f%%). Card is now marked as used.\n",
                   Members[i].mDisc);
            return discount;
        }
    }
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• STAFF PORTAL â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
void staff() {
    int choice;
    system("cls");
    printHeader("AYH MART  |  Surat, India");
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• FILE HANDLING â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
/* â”€â”€ Save Products â”€â”€ */
void saveProducts() {
    FILE *fp = fopen(FILE_PRODUCTS, "wb");
    if (!fp) {
        printf("  [âœ—] Error: Could not open products file for writing.\n");
        return;
    }
...
void loadProducts() {
    ...
    if (!fp) return; /* no save file yet â€” keep defaults */
    fread(&nProducts, sizeof(int), 1, fp);
    fread(shop, sizeof(struct Product), nProducts, fp);
    fclose(fp);
}

/* â”€â”€ Save Members â”€â”€ */
void saveMembers() {
    FILE *fp = fopen(FILE_MEMBERS, "wb");
    if (!fp) {
        printf("  [âœ—] Error: Could not open members file for writing.\n");
        return;
    }
...
/* â”€â”€ Save Customers â”€â”€ */
void saveCustomers() {
    FILE *fp = fopen(FILE_CUSTOMERS, "wb");
    if (!fp) {
        printf("  [âœ—] Error: Could not open customers file for writing.\n");
        return;
    }
...
void saveSales() {
    ...
    if (!fp) {
        printf("  [âœ—] Error: Could not open sales file for writing.\n");
        return;
    }
...
/* â”€â”€ Backup & Restore Menu (Admin) â”€â”€ */
void backupRestore() {
    int choice;
    system("cls");
    printHeader("AYH MART  |  Surat, India");
