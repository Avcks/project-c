...
struct Member {
    ...
    char  mTier[20];
    float mDisc;
    int   mUsed;   /* ── NEW: 0 = card active, 1 = card already used ── */
} Members[MAX_MEMBERS];
int nMembers = 0;
...
void addMembers() {
    ...
    for (i = 0; i < count; i++) {
        ...
        Members[nMembers].mUsed = 0;   /* ── NEW: card starts as unused ── */
        nMembers++;
    }
    ...
    if (nMembers == 0) {
        ...
        for (i = 0; i < nMembers; i++) {
            printf("  %-5d  %-18s  %-10.2f  %-10s  %-9.0f%%  %s\n",
                   ...
                   Members[i].mTier,
                   Members[i].mDisc,
                   Members[i].mUsed ? "Used" : "Active");   /* ── NEW ── */
        }
    }
    printf("\n");
    system("pause");
...
float applyDiscount(int memberId, float billAmount) {
    int i;
    if (memberId == 0) return 0;   /* ── no member entered, skip silently ── */

    for (i = 0; i < nMembers; i++) {
        if (Members[i].mId == memberId) {

            /* ── NEW: block reuse of an already-used card ── */
            if (Members[i].mUsed == 1) {
                printf("\n  [!] Membership card already used. No discount applied.\n");
                return 0;
            }

            /* ── card is valid: apply discount then permanently mark as used ── */
            float discount = billAmount * (Members[i].mDisc / 100.0f);
            Members[i].mSpent += (billAmount - discount);
            totalDiscount     += discount;
            Members[i].mUsed   = 1;   /* ── NEW: lock the card after one use ── */
...
/* ── discountReport: now shows card status too ── */
void discountReport() {
    int i;
    system("cls");
