...
void printHeader(const char *title) {
    ...
    printf(BOLD BWHITE "%s" RESET, title);
    for (i = 0; i < totalWidth - len - leftPad; i++){
        printf(" ");
    } 
...
void view() {
    ...
    if (nProducts == 0) {
        ...
        printDash();
        for (i = 0; i < nProducts; i++) {
            printf("    " BCYAN "%-6d" RESET "  %-18s  " BGREEN "%-8.2f" RESET "  ", shop[i].pCode, shop[i].pName, shop[i].pPrice);
            if (shop[i].pStock <= 5){
...
void showCustomerHistory(int custId) {
    ...
    for (i = 0; i < nSales; i++) {
        if (sales[i].custId == custId) {
            printf("    " BCYAN "%-6d" RESET "  %-18s  %-5d  " BGREEN "%.2f\n" RESET, sales[i].pCode, sales[i].pName, sales[i].qty, sales[i].total);
...
void addMembers() {
    ...
    for (i = 0; i < nCustomers; i++) {
        if (customers[i].cMemberId == 0){
            printf("    " BCYAN "%-6d" RESET "  %-18s  " BYELLOW "None\n" RESET, customers[i].cId, customers[i].cName);
        }
        else{
            printf("    " BCYAN "%-6d" RESET "  %-18s  " BGREEN "%d\n" RESET, customers[i].cId, customers[i].cName, customers[i].cMemberId);
        }
    }
    printDash();
...
void viewMembers() {
    ...
    if (nMembers == 0) {
        ...
        printDash();
        for (i = 0; i < nMembers; i++) {
            const char *tierColor = (Members[i].mTier[0] == 'P') ? BCYAN : (Members[i].mTier[0] == 'G') ? BYELLOW : WHITE;
            const char *statusColor = Members[i].mUsed ? BRED : BGREEN;

            printf("    " BCYAN "%-5d" RESET "  %-18s  %-10.2f  %s%-10s" RESET "  %-9.0f%%  %s%s" RESET "\n", Members[i].mId, Members[i].mName, Members[i].mSpent, tierColor, Members[i].mTier, Members[i].mDisc, statusColor, Members[i].mUsed ? "Used" : "Active");
        }
    }
    printf("\n");
    system("pause");
...
void viewCustomers() {
    ...
    if (nCustomers == 0) {
        ...
        printf("    " BOLD BYELLOW "%-6s  %-15s  %-12s  %-15s  %-25s  %s\n" RESET, "ID", "NAME", "PASSWORD", "PHONE", "EMAIL", "MEMBER ID");
        printDash();
        for (i = 0; i < nCustomers; i++) {
            char masked[20];
            ...
            if (customers[i].cMemberId != 0) {
                printf("    " BCYAN "%-6d" RESET "  %-15s  " BYELLOW "%-12s" RESET "  %-15s  %-25s  " BGREEN "%d\n" RESET, customers[i].cId, customers[i].cName, masked, customers[i].cPhone, customers[i].cEmail, customers[i].cMemberId);
            } else {
                printf("    " BCYAN "%-6d" RESET "  %-15s  " BYELLOW "%-12s" RESET "  %-15s  %-25s  None\n", customers[i].cId, customers[i].cName, masked, customers[i].cPhone, customers[i].cEmail);
            }
        }
    }
    printf("\n");
    ...
    if (linkedId == 0) {
        ...
        for (i = 0; i < nMembers; i++) {
            if (Members[i].mId == linkedId) {
                ...
                printf("    " BCYAN "Card Status    : " RESET "%s\n", Members[i].mUsed ? BRED "Used (discount already redeemed)" RESET : BGREEN "Active (discount available)" RESET);
                break;
            }
        }
        if (!found) {
...
float applyDiscount(int memberId, float billAmount) {
    ...
    for (i = 0; i < nMembers; i++) {
        if (Members[i].mId == memberId) {
            ...
            Members[i].mUsed   = 1;
            printf("    " BGREEN "[OK]" RESET " Membership discount applied ("BGREEN "%.0f%%" RESET "). Card is now marked as " BRED "used" RESET ".\n", Members[i].mDisc);
            return discount;
        }
    }
...
void discountReport() {
    ...
    if (nMembers == 0) {
        ...
        printf("    " BOLD BYELLOW "%-5s  %-18s  %-10s  %-10s  %-9s  %s\n" RESET, "ID", "NAME", "SPENT", "TIER", "DISCOUNT", "CARD STATUS");
        printDash();
        for (i = 0; i < nMembers; i++) {
            const char *tierColor = (Members[i].mTier[0] == 'P') ? BCYAN : (Members[i].mTier[0] == 'G') ? BYELLOW : WHITE;
            const char *statusColor = Members[i].mUsed ? BRED : BGREEN;
        ...
        switch (choice) {
            case 1: {
                ...
                if (nProducts == 0) {
                    ...
                    printf("    " BOLD BYELLOW "%-6s  %-18s  %-8s  %-6s  %s\n" RESET, "CODE", "NAME", "PRICE", "STOCK", "CATEGORY");
                    printDash();
                    for (i = 0; i < nProducts; i++)
                        printf("    " BCYAN "%-6d" RESET "  %-18s  " BGREEN "%-8.2f" RESET "  %-6d  %s\n", shop[i].pCode, shop[i].pName, shop[i].pPrice, shop[i].pStock, shop[i].pCategory);
                }
            ...
            case 2: {
                ...
                if (nMembers == 0) {
                    ...
                    for (i = 0; i < nMembers; i++) {
                        ...
                        printf("    " BCYAN "%-5d" RESET "  %-18s  %-10.2f  %-10s  %-9.0f%%  %s%s" RESET "\n", Members[i].mId, Members[i].mName, Members[i].mSpent, Members[i].mTier, Members[i].mDisc, sc, Members[i].mUsed ? "Used" : "Active");
                    }
                }
                printf("\n"); 
                system("pause"); 
...
void loadCustomers() {
    ...
    if (!fp){
        ERR("Could not load customers.");
        return;
    } 
    fread(&nCustomers, sizeof(int), 1, fp);
    if (nCustomers > MAX_CUSTOMERS){
        nCustomers = MAX_CUSTOMERS;
    } 
...
int isProductNameExists(char name[], int excludeIndex) {
    ...
    for (i = 0; i < nProducts; i++) {
        ...
        if (strEqualCI(shop[i].pName, name)){
            return 1;
        } 
    }
    return 0;
