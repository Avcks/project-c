...
struct Member {
    ...
    char  mTier[20];
    float mDisc;
    int   mUsed;   /* 0 = card active, 1 = card already used */
} Members[MAX_MEMBERS];
int nMembers = 0;
...
struct Customer {
    ...
    char cName[50];
    char cPass[20];
    int  cMemberId;   /* ── NEW: linked Member ID, 0 = no membership ── */
...
void addCustomer() {
    ...
    for (i = 0; i < count; i++) {
        ...
        /* ── NEW: link a Member ID to this customer account (0 = none) ── */
    ...
    if (nCustomers == 0) {
        ...
        for (i = 0; i < nCustomers; i++) {
            printf("  %-6d  %-18s  %-12s  %s\n",
                   ...
                   customers[i].cName,
                   customers[i].cPass,
                   customers[i].cMemberId != 0
                       ? (printf(""), /* trick: just fall through to %d */
                          "")         /* placeholder replaced below */
                       : "None");
            /* reprint the last row properly when cMemberId != 0 */
        }
    }
    printf("\n");
    system("pause");
...
/* ══════════════════ VIEW MY MEMBERSHIP CARD (NEW) ══════════════════ */
/*
 * Called from the Customer menu (option 6).
 * Uses the currently-logged-in customer's linked cMemberId to locate
 * their membership record and display all card details on-screen.
 * The customer is already authenticated via customerLogin(), so no
 * second password prompt is needed.
 */
void viewMyCard() {
    int i, found = 0;
    ...
    /* ── show the customer's own account info at the top ── */
