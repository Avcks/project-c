...
/* ── NEW: Customer account forward declarations ── */
void custManage();
void addCustomer();
void deleteCustomer();
void viewCustomers();
int  customerLogin();   /* returns index of logged-in customer, -1 on failure */
...
/* ── NEW: Customer account structure ── */
struct Customer {
    int  cId;
    char cName[50];
    char cPass[20];
...
int nCustomers   = 0;
int custLoggedIn = 0;   /* 1 = a customer is currently logged in */
int currentCustIdx = -1; /* index into customers[] of logged-in customer */
...
/* ══════════════════ CUSTOMER LOGIN (NEW) ══════════════════ */
/*
 * Prompts the customer for their ID and password.
 * Returns the index in customers[] on success, -1 on failure.
 * Sets the globals custLoggedIn and currentCustIdx on success.
 */
int customerLogin() {
    int  id, i;
    char pass[20];
...
void cust() {
    ...
    /* ── NEW: require login before entering self-checkout ── */
    if (!custLoggedIn) {
        if (customerLogin() == -1) {
            mainManu();
            return;
    ...
    printf("  Welcome, %s\n", customers[currentCustIdx].cName);  /* ── NEW ── */
    printDash();
...
void checkout() {
    ...
    if (payChoice == 2) {
        ...
        printf("  |                                                      |\n");
        printf("  +------------------------------------------------------+\n");
        printf("  UPI ID : 6353999107@ptyes\n");
        printf("  Amount : Rs. %.2f\n\n", finalAmount);
        totalQRPayments++;
    ...
    /* ── NEW: log out customer after successful checkout ── */
    custLoggedIn   = 0;
    currentCustIdx = -1;
}
...
/* ── Customer management menu (Admin & Staff) ── */
void custManage() {
    int choice;
    system("cls");
    printHeader("AYH MART  |  Surat, India");
...
void staff() {
    ...
    switch (choice) {
        ...
        case 7: {                        /* ── NEW: staff customer account management ── */
            int staffChoice;
            system("cls");
            printHeader("AYH MART  |  Surat, India");
            printHeader("CUSTOMER ACCOUNT MANAGEMENT");
